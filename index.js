...
async function askAI(guildId, channelId, userName, userMessage) {
  ...
  const systemText =
    "You are Shinra Katsumi, a Discord server AI assistant. " +
    "Your personality is playful, chill, cheerful, slightly mischievous and childish-but-mature. " +
    "Reply naturally instead of using canned/sample replies. Understand and reply in the language the user uses, including Hinglish and common multilingual chat. " +
    "You may joke and tease lightly, but keep everything non-explicit and suitable for a teen-friendly Discord server. " +
    "Do not claim to be human. Keep normal Discord replies concise, but give useful detail when asked. " +
    "Remember recent conversation context when it helps.";

  const contents = [
    { role: "user", parts: [{ text: systemText }] },
    { role: "model", parts: [{ text: "Got it. I'll respond naturally as Shinra Katsumi." }] },
  ...
  try {
    ...
    if (!response.ok) {
      console.error("[AI] Gemini API error:", response.status, JSON.stringify(data).slice(0, 1500));
      return null;
    }
...
client.once("ready", () => {
  ...
  console.log(`Gemini AI configured: ${GEMINI_API_KEY ? "yes" : "no (set GEMINI_API_KEY on Render)"}`);
});
...
async function runCommand(msg, cmd, args) {
  ...
  try {
    ...
    if (cmd === "ai") {
      ...
      if (action === "enable") {
        if (!GEMINI_API_KEY) return sendEmbed(msg, "AI mode", "AI API key is not configured. Add `GEMINI_API_KEY` in Render Environment Variables.");
